...
switch expIDX
    case {'1A','1B'} % Passive
        condsName = {'STEP','CONST','REG','REGRAND','RAND','RANDREG'};
        statpairs = [1,2;3,4;5,6];
        P = P(:,[1,2,4,3,6,5]);
        condsName = condsName([1,2,4,3,6,5]);
    case {'3A','3B'} % ACTIVE task
        condsName = {'CONST','STEP','REG','REGRAND','RAND','RANDREG'};
        statpairs = [1,2;3,4;5,6];
        P = P(:,[2,1,4,3,6,5]);
        condsName = condsName([2,1,4,3,6,5]);
end
...
if clusterstats
    ...
    for k = 1:size(statpairs,1)
        ...
        % _____ PLOT______
        hold on;
        %             for i = 1:size(neg,1);line([timeaxis(neg(i,1)) timeaxis(neg(i,2))],[Ystat Ystat],'LineWidth',4,'Color',colourmap2(k,:)); end
        %             for i = 1:size(pos,1);line([timeaxis(pos(i,1)) timeaxis(pos(i,2))],[Ystat Ystat],'LineWidth',4,'Color',colourmap2(k,:)); end
